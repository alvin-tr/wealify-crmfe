  ...
  <div v-if="show" class="metric-builder-overlay" @click.self="$emit('close')">
    <div class="metric-builder-modal">
      ...
      <div class="mb-body">
        ...
        <!-- Step 5½: MoM Comparison -->
        <div class="mb-field" v-if="form.operation">
          <label>So sánh kỳ trước (MoM)</label>
          <div class="mb-mom-row">
            <label class="mb-toggle">
              <input type="checkbox" v-model="form.enableMoM" />
              <span class="mb-toggle-label">Bật so sánh Tháng trước</span>
            </label>
            <select v-if="form.enableMoM" v-model="form.dateColumn" class="mb-select sm">
              <option value="">— Cột ngày —</option>
              <option v-for="c in dateColumns" :key="c.column" :value="c.column">{{ c.label }} ({{ c.column }})</option>
            </select>
          </div>
        </div>
...
const dateColumns = computed(() => {
  const cols = currentColumns.value.filter((c: any) => c.column !== '*');
  // Suggest date-like columns first
  const datish = ['created_at', 'registered_at', 'updated_at', 'last_tx_at'];
  return cols.sort((a: any, b: any) => {
    const ai = datish.includes(a.column) ? -1 : 1;
    const bi = datish.includes(b.column) ? -1 : 1;
    return ai - bi;
  });
});
...
async function testMetric() {
  ...
  try {
    ...
    if (form.value.enableMoM && form.value.dateColumn) {
      body.dateColumn = form.value.dateColumn;
      body.comparePeriod = 'MoM';
    }
...
function addMetric() {
  ...
  const metric = {
    ...
    dateColumn: (form.value.enableMoM && form.value.dateColumn) ? form.value.dateColumn : undefined,
    comparePeriod: (form.value.enableMoM && form.value.dateColumn) ? 'MoM' as const : undefined,
  };
  if (props.editMetric) {
    emit('update', metric);
  } else {
  ...
  // Reset form
  form.value = { table: '', column: '', operation: '', filterColumn: '', filterOperator: '=', filterValue: '', multiplier: null, label: '', format: 'number', icon: '📊', enableMoM: false, dateColumn: '', comparePeriod: '' };
  previewResult.value = null;
  previewError.value = '';
  emit('close');
}
...
.mb-mom-row { display: flex; gap: 12px; align-items: center; flex-wrap: wrap; }
.mb-toggle { display: flex; align-items: center; gap: 8px; cursor: pointer; }
.mb-toggle input[type="checkbox"] { accent-color: #7c3aed; width: 16px; height: 16px; }
.mb-toggle-label { font-size: .82rem; color: #e2e8f0; font-weight: 500; }
